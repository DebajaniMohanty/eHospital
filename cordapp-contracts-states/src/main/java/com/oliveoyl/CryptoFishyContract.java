...
public class CryptoFishyContract implements Contract {
    ...
    private void isFishy(LedgerTransaction tx) throws IllegalArgumentException {
        ...
        if (commands.size() != 1) throw new IllegalArgumentException("There must be exactly one command");
        CryptoFishyCommands command = (CryptoFishyCommands) commands.get(0).getValue();

        if (command instanceof CryptoFishyCommands.Issue) verifyIssue(tx);
        else if (command instanceof CryptoFishyCommands.Fish) verifyFishCommand(tx);
        else if (command instanceof CryptoFishyCommands.TransferFished) verifyTransferFished(tx);
        else if (command instanceof CryptoFishyCommands.TransferUnfished) verifyTransferUnfished(tx);
